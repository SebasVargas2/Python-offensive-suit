...
def get_arguments():
    parser = argparse.ArgumentParser(description="Herramienta para cambiar la direccion MAC de una dirreccion de red")
...
def is_valid_input(interface, mac_address):
    ...
    is_valid_interface = re.match(r'^[e][n|t][s|h|p]\d{1}', interface)
    is_valid_mac_adress = re.match( r'^([A-Fa-f0-9]{2}[:]){5}[A-Fa-f0-9]{2}$' ,mac_address)

    return is_valid_interface and is_valid_mac_adress
