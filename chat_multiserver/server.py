...
def cliente_thread(client_socket, clients, usernames):
    ...
    for client in clients:
        if client is not client_socket:
            client.sendall((f'\n[!] el usuario {username} ha ingresado al chat\n').encode())
...
def server_program():
    ...
    while True:
        ...
        thread = threading.Thread(target=cliente_thread, args=(client_socket, clients, usernames))
        thread.daemon = True
        thread.start()
